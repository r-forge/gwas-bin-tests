...
#'
#' cf \link{concept#allelic.sum.score}
...
#' @export
simpleAllelicSumScores <- function(genotypes_list,  phenotypes, covariables=data.frame(), regCoeff=0) {
	...
	sum( sapply(genotypes_list, function(g) {
		scores <- by(data.frame(g,phenotypes), covariables, function(x) {
			...
			all_ct <- matrix(0L, nrow = 2, ncol = 2)



			row.names(all_ct) <- c('a', 'A')
			all_ct['a',] <- 2*ct['aa',] + ct['Aa',]
			all_ct['A',] <- 2*ct['AA',] + ct['Aa',]
			expected <- ( rowSums(all_ct) %o% colSums(all_ct) ) / sum(all_ct)
			m <- (all_ct-expected)^2 / expected
...
#'
#' See \code{\link{simpleAllelicSumScores}}
#' This is a reference implementation, very naive on purpose, optimized for clarity.
#'
...
#' @export
simpleAllelicSumScoresPvalue <- function(nb_permutations,genotypes_list,  phenotypes,
		covariables=data.frame(), regCoeff=0) {

	n <- length(phenotypes)

	observed <- simpleAllelicSumScores(genotypes_list, phenotypes, covariables, regCoeff)
	...
	scores <- sapply(1:nb_permutations, function(x) {
				...
				simpleAllelicSumScores(genotypes_list, phenotypes, covariables, regCoeff)
			})
