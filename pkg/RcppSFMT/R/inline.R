...
CxxFlags <- function( print = TRUE ){
	incs <- system.file(c('include', 'include/SFMT-src'), package="RcppSFMT")
	flags <- paste("-I", incs, sep='', collapse=' ')
	flags <- paste(flags, SIMD_FLAGS, sep=' ')
    if( print) cat( flags ) else flags
}

WinCxxFlags <- function( print = TRUE ){
	incs <- system.file(c('include/winconfig', 'include', 'include/SFMT-src'), package="RcppSFMT")
	flags <- paste("-I", incs, sep='', collapse=' ')
	flags <- paste(flags, SIMD_FLAGS, sep=' ')
    if( print) cat( flags ) else flags
}
